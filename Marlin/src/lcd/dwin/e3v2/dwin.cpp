...
void Item_Prepare_Home(const uint8_t row) {
  ...
  void Item_Prepare_Offset(const uint8_t row) {
    if (HMI_IsChinese()) {
      ...
      #else
        DWIN_Frame_AreaCopy(1, 43, 87, 98, 101, LBLX, MBASE(row));
      #endif
    }
    else {
      #if HAS_BED_PROBE
...
void draw_steps_per_mm(const uint16_t line) {
  DWIN_Frame_AreaCopy(1, 1, 151, 120, 161, LBLX, line);   // "Steps-per-mm"
}
...
void Draw_Printing_Screen()
{
  ...
  {
    // DWIN_Frame_AreaCopy(1, 40,  2,  92, 13,  14,   9);  // Tune Printing
    DWIN_Draw_String(false, false, font8x16, Color_White, Color_Bg_Black, 14, 9, F("Printing"));
...
void Popup_window_Filament(void)
{
  ...
  else
  {
    ...
    DWIN_Draw_String(false,true,font8x16, Popup_Text_Color, Color_Bg_Window, 20, 170, (char*)"please replacce the filament,");
...
void HMI_Filament()
{
  ...
  if (encoder_diffState == ENCODER_DIFF_NO) return;
      ...
      if(HMI_flag.select_flag)
      {
        // SERIAL_ECHOLNPAIR("READ(CHECKFILAMENT_PIN)111=: ", READ(CHECKFILAMENT_PIN));
        if(READ(CHECKFILAMENT_PIN) == 0)//原为if(READ(CHECKFILAMENT_PIN))------zy
        {
          delay(200); //防止抖动  rock_20210910
          // SERIAL_ECHOLNPAIR("READ(CHECKFILAMENT_PIN)2222=: ", READ(CHECKFILAMENT_PIN));
            if(READ(CHECKFILAMENT_PIN) == 0)//原为if(READ(CHECKFILAMENT_PIN))------zy
            {
              HMI_flag.cutting_line_flag=false;
              temp_cutting_line_flag=false;
              // SERIAL_ECHOLNPAIR("HMI_flag.remove_card_flag22=: ", HMI_flag.remove_card_flag);
              // SERIAL_ECHOLNPAIR("temp_remove_card_flag22=: ", temp_remove_card_flag);
...
void HMI_Control()
{
  ...
  void HMI_Leveling() {
    ...
    DWIN_UpdateLCD();
    thermalManager.disable_all_heaters();  //rock_20210910
    /* see "Level" button changes for UBL in https://www.mironv.com/2020/08/14/ender3v2-bltouch-marlin/
     * changed commands are
     *
     * M140 S65 ; Warm bed to 65C.
     * M190 S65 ; Wait until bed is warm so we're more accurate.
     * G28 ; Home XYZ.
        ...
        case TEMP_CASE_PLA: { // PLA preheat setting
          ...
          if (HMI_IsChinese()) {
            ...
            DWIN_Frame_AreaCopy(1, 1, 134, 56, 146, LBLX + 24, MBASE(PREHEAT_CASE_TEMP));     // PLA nozzle temp
              ...
              DWIN_Frame_AreaCopy(1, 197, 104, 240, 116, LBLX + 27, MBASE(PREHEAT_CASE_TEMP));
...
void Check_Filament_Update(void)
{
  ...
  {
  if(card.isPrinting() && (!temp_cutting_line_flag) || HMI_flag.cloud_printing_flag)//SD卡打印或者云打印
    if(millis() - lMs > 500)
    {
        lMs = millis();
        /* exist material */
...
void EachMomentUpdate()
{
 ...
 // cutting after homing
  if (HMI_flag.cutting_line_flag && printingIsPaused() && !planner.has_blocks_queued())
  {
    HMI_flag.cutting_line_flag = false;
    #if ENABLED(PAUSE_HEAT)
      TERN_(HAS_HOTEND, resume_hotend_temp = thermalManager.degTargetHotend(0));
